...
- Доп. проверка «пустые блоки при скрытии» (ПИ, программная gap-детекция + крайние паттерны скрытия, полный откат): дыр нет, код не менялся — детали в `plans/hiding_mechanism.md` §9: **Executed**, 2026-09-04.

## 8. Риски

| # | Риск | Митигация |
|---|---|---|
| R17 | «Пустые блоки/дыры» по вертикали при скрытии событий в середине списка (промежуточные дети вне wrapper/separator, марджи соседей, pinned-placeholder, виртуальная высота скроллера) | **Проверено на ПИ 2026-09-04 — дыр нет** (`hiding_mechanism.md` §9): программная gap-детекция >40px по соседним видимым нодам + крайние паттерны (месяц целиком, ≥8 подряд, верх до load-more, grouped-complex) → 0 аномальных зазоров; инвентарь детей = только wrapper/separator. Остаточный риск: новый тип прямого ребёнка от amo не скрывается проходом → повторить сценарий §9 при изменении инвентаря |
