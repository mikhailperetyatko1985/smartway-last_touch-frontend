...
## 9. Пустые блоки при скрытии — проверка 2026-09-04

Задача: доказать, что при фильтрации в середине списка НЕ возникает незаполненных пустых блоков/дыр по вертикали; если возникают — найти DOM-вину и зафиксировать. Метод: chrome-devtools MCP на деталке 42661594 (read-only + временные inline-style с полным откатом), реплика `classify`/`hideNode`/`processSeparators` из кода, target set B = {10722265,10136549,12830901,13159601}, cfg дефолт (все `hide_*=true`, в т.ч. `hide_pinned_no_target=true`).

**Исходный инвентарь на момент проверки:** 348 прямых детей `.js-notes` = 335 wrapper'ов + 13 разделителей; **иных типов детей нет (0)** — все покрыты `isEventWrapper/isSeparator`. DOM подрос с момента research (346→348, разделители 12→13). Пinned-группа лежит в начале списка: каждая pinned-заметка предшествует собственным month-разделителем («Апрель»@idx0 + 2 pinned, «Июнь»@idx3 + 1 pinned), затем хронология с idx5.

**Результаты по случаям (дыр НЕ найдено):**

| # | Сценарий | Числа / результат |
|---|---|---|
| Основной проход | allShown→filtered, реплика кода | скрыто **206/348** wrapper'ов (совпадает с dry-run 206), разделителей видны 9/13; видимых детей 138; **зазоров >40px между соседними видимыми нодами: 0**; макс. зазор = 20px |
| 2а/б | Программная детекция + DOM-вина каждого gap | Список gap'ов пуст. Единственные gaps >40px в системе — это amo-собственный `margin-bottom:73px` у баров `-opened_talks` (4 шт.), **присутствуют ещё до любого скрытия** и исчезают вместе с нодами при их HIDE (bar'ы → P3 no-author) |
| 2в | scrollHeight до/после + обратный toggle | list `scrollHeight` 33118→15717, scroller 33577→16176 — точно следуют за суммой видимых высот (+марджи), виртуальной/фиксированной высоты нет (у scroller лишь inline `max-height:870px`). Обратный toggle через реальный путь `showNode`/`unhideAll`: cssText всех 348 детей **побайтово идентичен** исходному снапшоту, `display:flex;height:1px` у `-opened_talks` и `display:block;height:{206,190,170}px` у pinned восстановлены точно |
| 2г | Pinned-стопка (принудительно скрыта СРЕДНЯЯ карта) | Placeholder 190px исчезает вместе с wrapper'ом — зазоров в flow нет; оверлеи не наезжают друг на друга; оставшиеся карточки держат исходные `top` до пересчёта amo-fixers (intentional sticky-зона у верха, по определению задачи исключается из flow-детекции) |
| 2д.i | Весь месяц «Июнь» между двумя разделителями (41 нода) скрыт | Разделитель «Июнь» скрыт, соседние («Май»,«Июль») видны; зазор на швах = только марджи. **Суммарная дыра = 0** — самый опасный кейс пройден |
| 2д.ii | ≥5 подряд в середине (принудительно скрыто 8 wrapper'ов) | Новых gap'ов нет, соседи по обе стороны зазоров не получают |
| 2д.iii | Верх до load-more целиком: pinned-группа + весь раздел «Апрель» (110 нод, все 3 pinned) | Первая видимая нода = разделитель «Май» в 15px от верха списка; gap'ов сверх baseline нет; оверлейная стопка пуста без артефактов |
| 2д.iv | `-call_in_out-grouped-complex` капсула целиком скрыта между видимыми соседями | Зазор вокруг = марджи соседей, дыры нет (Q2: один вердикт на весь wrapper) |
| 2е | Скриншоты зон до/после/отката | `plans/research_screens/pi_05_filtered_top.png`, `pi_06_filtered_mid.png`, `pi_07_filtered_bottom.png` (filtered, зоны верх/середина/низ — непрерывный таймлайн без дыр), `pi_08_after_final_rollback.png` (идентичен исходному состоянию) |
| Откат | Инвентарь-диф после каждого эксперимента и финально | 348 детей; `[data-stf-hidden]`=0 и `[data-stf-prev-display]`=0 по всему документу; `cssText`-диф vs снапшот = **0**; computed `display:none` в списке = 0 |
| Догрузка (2.3) | Скролл к верху с фильтром активным | История исчерпана (`load-more` h=0, новых нод нет); повторный проход идемпотентен (206 скрыто / 9 разделителей видны — идентично первому проходу). Прямые чеки: «видимый разделитель над пустой секцией» = **0**, «ложно-скрытый разделитель при видимых событиях ниже» = **0** |

**Вывод:** механизм `hideNode + processSeparators` не создаёт пустых блоков в середине списка. Все четыре гипотетических источника дыр проверены и исключены: (1) промежуточные дети вне wrapper/separator — отсутствуют в живом DOM; (2) марджи видимых соседей — макс. 20px после скрытия (amo-марджин `opened_talks` уезжает вместе с nodой); (3) placeholder/pinned-зона — исчезает вместе с wrapper'ом, оверлеи не наезжают; (4) виртуальная высота скроллера — отсутствует.

**Оставшиеся наблюдения (без фикса):**
- Если amo в будущем добавит в `.js-notes` новый тип прямого ребёнка (не wrapper/separator), он не будет скрываться проходом и может остаться «сиротой» между скрытыми событиями. Канарейка покрывает только полную переверстку якорей. Повторная проверка этого сценария при любом изменении инвентаря типов обязательна; расширение `processWrappers` на прочие прямые дети — только при фактическом наблюдении.
- Позиции pinned-оверлеев после скрытия одной из карточек стопки пересчитываются amo-fixers не ранее следующего скролла/перерисовки (в окне между — визуальный просвет в sticky-колонке без наложений); flow-дыр это не создаёт.
